...
const generator = () => {
let randomSentenceStart = sentenceStart[Math.floor(Math.random() * sentenceStart.length)];
let randomMidSentence = midSentence[Math.floor(Math.random() * midSentence.length)];
let randomSentenceEnd = sentenceEnd[Math.floor(Math.random() * sentenceEnd.length)];
return `${randomSentenceStart} ${randomMidSentence}! What you must do now is ${randomSentenceEnd}`
}
